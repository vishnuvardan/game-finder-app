...
class GeminiService {
     ...
     */
    async generateContentWithFallback(contents, systemInstruction, responseSchema) {
        const models = [
            'gemini-2.5-flash',
            'gemini-3.5-flash',
            'gemini-flash-latest',
     ...
     */
    async generateRetrospectiveDepartments(gameName, genres) {
        const prompt = `The user has just finished the game: "${gameName}" within genres: ${genres.join(', ')}.`;
        const systemInstruction = `You are a critical video game reviewer and taxonomist. The user has just finished the game: "${gameName}" within genres: ${genres.join(', ')}. ` +
     ...
     */
    async generateReviewDraft(reviewerName, gameName, ratings) {
      ...
      ${ratingsSummary}
    `;
        const systemInstruction = `You are a witty, highly articulate gamer writing a short retrospective review post for social media (Reddit/X). ` +
